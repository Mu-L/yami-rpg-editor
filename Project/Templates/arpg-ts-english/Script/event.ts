...
let EventManager = new (class GlobalEventManager {
	...
	public initialize(): void {
		...
		Scene.on('keydown', (event: ScriptKeyboardEvent) =>
			this.emit('keydown', { priority: false, argument: event })
		)
		Scene.on('keyup', (event: ScriptKeyboardEvent) =>
			this.emit('keyup', { priority: false, argument: event })
		)
		Scene.on('mousedown', (event: ScriptMouseEvent) =>
			this.emit('mousedown', { priority: false, argument: event })
		)
		Scene.on('mouseup', (event: ScriptMouseEvent) =>
			this.emit('mouseup', { priority: false, argument: event })
		)
		Scene.on('mousemove', (event: ScriptMouseEvent) =>
			this.emit('mousemove', { priority: false, argument: event })
		)
		Scene.on('doubleclick', (event: ScriptMouseEvent) =>
			this.emit('doubleclick', { priority: false, argument: event })
		)
		Scene.on('wheel', (event: ScriptWheelEvent) =>
			this.emit('wheel', { priority: false, argument: event })
		)
		Scene.on('touchstart', (event: ScriptTouchEvent) =>
			this.emit('touchstart', { priority: false, argument: event })
		)
		Scene.on('touchmove', (event: ScriptTouchEvent) =>
			this.emit('touchmove', { priority: false, argument: event })
		)
		Scene.on('touchend', (event: ScriptTouchEvent) =>
			this.emit('touchend', { priority: false, argument: event })
		)
		Scene.on('gamepadbuttonpress', (event: ScriptGamepadEvent) =>
			this.emit('gamepadbuttonpress', {
				priority: false,
				argument: event
			})
		)
		Scene.on('gamepadbuttonrelease', (event: ScriptGamepadEvent) =>
			this.emit('gamepadbuttonrelease', {
				priority: false,
				argument: event
			})
		)
		Scene.on('gamepadleftstickchange', (event: ScriptGamepadEvent) =>
			this.emit('gamepadleftstickchange', {
				priority: false,
				argument: event
			})
		)
		Scene.on('gamepadrightstickchange', (event: ScriptGamepadEvent) =>
			this.emit('gamepadrightstickchange', {
				priority: false,
				argument: event
			})
		)
		Input.on(
			...
			(event: ScriptKeyboardEvent) =>
				this.emit('keydown', { priority: true, argument: event }),
			true
		)
		Input.on(
			'keyup',
			(event: ScriptKeyboardEvent) =>
				this.emit('keyup', { priority: true, argument: event }),
			true
		)
		Input.on(
			'mousedown',
			(event: ScriptMouseEvent) =>
				this.emit('mousedown', { priority: true, argument: event }),
			true
		)
		Input.on(
			'mouseup',
			(event: ScriptMouseEvent) =>
				this.emit('mouseup', { priority: true, argument: event }),
			true
		)
		Input.on(
			'mousemove',
			(event: ScriptMouseEvent) =>
				this.emit('mousemove', { priority: true, argument: event }),
			true
		)
		Input.on(
			'doubleclick',
			(event: ScriptMouseEvent) =>
				this.emit('doubleclick', { priority: true, argument: event }),
			true
		)
		Input.on(
			'wheel',
			(event: ScriptWheelEvent) =>
				this.emit('wheel', { priority: true, argument: event }),
			true
		)
		Input.on(
			'touchstart',
			(event: ScriptTouchEvent) =>
				this.emit('touchstart', { priority: true, argument: event }),
			true
		)
		Input.on(
			'touchmove',
			(event: ScriptTouchEvent) =>
				this.emit('touchmove', { priority: true, argument: event }),
			true
		)
		Input.on(
			'touchend',
			(event: ScriptTouchEvent) =>
				this.emit('touchend', { priority: true, argument: event }),
			true
		)
		Input.on(
			'gamepadbuttonpress',
				...
				if (event.type === 'independent') {
					...
					if (independent.debugTimeout === undefined) {
						...
						if (independent.debugTimeout >= 60000) {
							...
							const initial =
								independent.initial as CommandFunctionList
...
class ScriptManager {
		...
		for (const instance of this.instances) {
			if (method in instance) {
				if (isSaveEvent) {
					...
					const define = ScriptManager.createSaveDefine(
						instance,
						pluginsContainer
					)
					...
					try {
						...
						if (returned !== undefined) {
							...
							if (
								...
								if (
									!argument.plugins ||
									typeof argument.plugins !== 'object'
								) {
									argument.plugins = pluginsContainer
								}
								Object.defineProperty(argument, 'plugins', {
	 ...
	 * @param value 返回值函数
	 */
	private static pushDeferredParameter(
		instance: object,
		key: string,
		value: Function
	): void {
	...
	public static loadDeferredParameters(): void {
		for (let i = 0; i < ScriptManager.deferredCount; i++) {
			ScriptManager.deferredInstances[i][ScriptManager.deferredKeys[i]] =
				ScriptManager.deferredValues[i]!()
	...
	private static createSaveDefine(
		instance: any,
		pluginsContainer: any
	): (key: string, value: AttributeValue) => void {
		...
		if (data.length > 0) {
			for (const wrap of data) {
				...
				if (wrap.initialized === undefined) {
					...
					wrap.parameters = ScriptManager.compileParamList(
						wrap.id,
						wrap.parameters
					)
				}
				...
				if (script === undefined) {
					...
					console.error(
						new Error(`The script is missing: ${name}`),
						owner
					)
					continue
				}
				...
				if (typeof guid === 'string' && guid !== '') {
					...
					if (
						!instanceDescriptor ||
						instanceDescriptor.configurable !== false ||
						instanceDescriptor.writable !== false ||
						instanceDescriptor.value !== guid
					) {
				...
				for (let i = 0; i < length; i += 2) {
					...
					if (typeof value === 'function') {
						if (ScriptManager.deferredLoading) {
							...
							ScriptManager.pushDeferredParameter(
								instance,
								key,
								value
							)
	 ...
	 * @returns 编译后的脚本参数列表
	 */
	public static compileParamList(
		id: string,
			...
			switch (type) {
				...
				case 'variable-number':
					if (typeof value !== 'number') {
						if (
							(value as ScriptVariableGetter)?.getter ===
							'variable'
						) {
							value = Command.compileVariable(
				...
				case 'variable-getter':
					if (
						(value as ScriptVariableGetter)?.getter === 'variable'
					) {
						value = Command.compileVariable(
							value as VariableGetter,
							Attribute.GET
				...
				case 'variable-setter':
					if (
						...
						value = {
							get: Command.compileVariable(
								value as VariableGetter,
								Attribute.GET
							),
							set: Command.compileVariable(
								value as VariableGetter,
								Attribute.SAFE_SET
							)
						}
				...
				case 'equipment-getter':
					if (
						(value as ScriptEquipmentGetter)?.getter === 'equipment'
					) {
						value = Command.compileEquipment(
				...
				case 'position-getter':
					if (
						(value as ScriptPositionGetter)?.getter === 'position'
					) {
						const getPoint = Command.compilePosition(
							value as PositionGetter
						)
						value = () => {
...
class ScriptTouchEvent {
	 ...
	 * @param event 脚本触摸事件
	 */
	public static *forEachElement(
		event: ScriptTouchEvent
	): Generator<ScriptTouchEvent> {
	 ...
	 * @param target 目标元素
	 */
	private static loadTargetTouches(
		event: ScriptTouchEvent,
		target: UIElement | null
	): void {
		event.target = target
	...
	/** {目标元素:触摸点编号列表}映射表 */
	private static targetToIdListMap: Map<UIElement | null, Array<number>> =
		new Map()
}
...
class TouchPoint {
	...
	constructor(touch: Touch) {
		...
		const { x: screenX, y: screenY } = Mouse.convertClientToScreenCoords(
			touch.clientX,
			touch.clientY
		)
		...
		const { x: sceneX, y: sceneY } = Mouse.convertScreenToSceneCoords(
			screenX,
			screenY
		)
		this.sceneX = sceneX
