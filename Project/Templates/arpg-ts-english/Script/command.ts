...
let Command = new (class CommandCompiler {
      ...
      switch (type) {
        ...
        case "self":
          return (value?: any) =>
            ...
            operation(SelfVariable.map, CurrentEvent.selfVarId, value);
        case "actor":
        case "skill":
        case "state":
        case "equipment":
  ...
  private compileConditions = (function (IIFE) {
    const { GET, BOOLEAN_GET, NUMBER_GET, STRING_GET, OBJECT_GET, LIST_GET } =
      Attribute;
          ...
          switch (operation) {
            case "present-active":
              return () => {
                ...
                return actor
                  ? Scene.actor === actor.parent && actor.active
                  : false;
              };
            case "present":
            ...
            case "is-team-friend":
              return () =>
                Team.isFriendly(getActor()?.teamId ?? "", condition.teamId);
            case "is-team-enemy":
              return () =>
                Team.isEnemy(getActor()?.teamId ?? "", condition.teamId);
          }
        }
        case "element": {
    ...
    const compileOperand = (operand: any): (() => number | undefined) => {
      switch (operand.type) {
        ...
        case "parameter": {
          const getKey = Command.compileString(operand.key);
          return () =>
            Command.getParameter(getKey(), "number") as number | undefined;
        }
        case "script":
          return Command.compileFunction(operand.script) as any;
        case "other":
    ...
    const compileOperand = (operand: any): (() => string | undefined) => {
      switch (operand.type) {
        ...
        case "parameter": {
          const getKey = Command.compileString(operand.key);
          return () =>
            Command.getParameter(getKey(), "string") as string | undefined;
        }
        case "script":
          return Command.compileFunction(operand.script) as any;
        case "other":
    ...
    return function ({
      ...
      if (length < 6) {
        ...
        switch (length) {
          ...
          case 2:
            return () => ((Value = getter()), a() ? f : b() ? g : defFn)();
          case 3:
            return () =>
              ((Value = getter()), a() ? f : b() ? g : c() ? h : defFn)();
          case 4:
            return () =>
              ((Value = getter()),
