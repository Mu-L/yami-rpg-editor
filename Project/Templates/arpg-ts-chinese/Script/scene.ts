...
let Scene = new (class SceneManager {
	...
	public initialize(): void {
		...
		Input.on('keydown', (event: ScriptKeyboardEvent) =>
			Scene.emitInputEvent('keydown', event)
		)
		Input.on('keyup', (event: ScriptKeyboardEvent) =>
			Scene.emitInputEvent('keyup', event)
		)
		Input.on('mousedown', (event: ScriptMouseEvent) =>
			Scene.emitInputEvent('mousedown', event)
		)
		Input.on('mouseup', (event: ScriptMouseEvent) =>
			Scene.emitInputEvent('mouseup', event)
		)
		Input.on('mousemove', (event: ScriptMouseEvent) =>
			Scene.emitInputEvent('mousemove', event)
		)
		Input.on('doubleclick', (event: ScriptMouseEvent) =>
			Scene.emitInputEvent('doubleclick', event)
		)
		Input.on('wheel', (event: ScriptWheelEvent) =>
			Scene.emitInputEvent('wheel', event)
		)
		Input.on('touchstart', (event: ScriptTouchEvent) =>
			Scene.emitInputEvent('touchstart', event)
		)
		Input.on('touchmove', (event: ScriptTouchEvent) =>
			Scene.emitInputEvent('touchmove', event)
		)
		Input.on('touchend', (event: ScriptTouchEvent) =>
			Scene.emitInputEvent('touchend', event)
		)
		Input.on('gamepadbuttonpress', (event: ScriptGamepadEvent) =>
			Scene.emitInputEvent('gamepadbuttonpress', event)
		)
		Input.on('gamepadbuttonrelease', (event: ScriptGamepadEvent) =>
			Scene.emitInputEvent('gamepadbuttonrelease', event)
		)
		Input.on('gamepadleftstickchange', (event: ScriptGamepadEvent) =>
			Scene.emitInputEvent('gamepadleftstickchange', event)
		)
		Input.on('gamepadrightstickchange', (event: ScriptGamepadEvent) =>
			Scene.emitInputEvent('gamepadrightstickchange', event)
		)
	}
...
class SceneContext {
	...
	constructor(data: SceneOptions = {}, savedData?: SceneSaveData) {
		...
		const ambient = savedData?.ambient ??
			data.ambient ?? { red: 255, green: 255, blue: 255, direct: 0 }
	...
	public getActor({
		...
		switch (area) {
			case 'square': {
				const half = size / 2
				const cells = this.actor.partition.get(
					x - half,
					y - half,
					x + half,
					y + half
				...
				for (let i = 0; i < length; i++) {
					for (const actor of cells[i] as Array<Actor>) {
						...
						if (
							distX <= half &&
							distY <= half &&
							inspector(actor.teamIndex, teamIndex)
						) {
		...
		if (duration > 0) {
			...
			updaters.set('ambient', {
				update: (deltaTime) => {
					...
					ambient.red = Math.clamp(
						sRed * (1 - time) + red * time,
						0,
						255
					)
					ambient.green = Math.clamp(
						sGreen * (1 - time) + green * time,
						0,
						255
					)
					ambient.blue = Math.clamp(
						sBlue * (1 - time) + blue * time,
						0,
						255
					)
	...
	public updateTerrain(x: number, y: number): void {
		...
		this.terrain.compositeArray[ti] =
			this.terrain.rawArray[ti] || this.parallax.getTileTerrain(x, y)
	}
	...
	public unloadSubscene(sceneId: string): void {
		const scene = this.subscenes.find((scene) => scene.id === sceneId)
		if (!scene) return
	...
	): SceneParticleEmitter | undefined {
		const data = node.data
		if (data) {
...
class SceneParallaxManager {
	...
	constructor(scene: SceneContext) {
		...
		this.groups = [this.backgrounds, this.foregrounds, this.doodads]
	}
			...
			if (preset) {
				...
				switch (preset.class) {
					case 'parallax':
						...
						this.append(
							new SceneParallax(
								savedData as unknown as SceneParallaxData
							)
						)
						continue
					case 'tilemap':
						...
						this.append(
							new SceneTilemap(
								scene,
								savedData as unknown as SceneTilemapData
							)
						)
						continue
				}
...
class SceneParallax {
	...
	public update(deltaTime: number): void {
		...
		if (this.shiftSpeedX !== 0 || this.shiftSpeedY !== 0) {
			if (this.texture?.complete) {
				this.shiftX =
					(this.shiftX +
						(this.shiftSpeedX * deltaTime) /
							1000 /
							this.texture.width) %
					1
				this.shiftY =
					(this.shiftY +
						(this.shiftSpeedY * deltaTime) /
							1000 /
							this.texture.height) %
					1
			}
	...
	public static lightSamplingModes: ParallaxLightSamplingMap = {
		raw: 0,
		global: 1,
		anchor: 2,
		ambient: 3
...
class SceneTilemap {
	...
	constructor(scene: SceneContext, tilemap: TilemapOptions = {}) {
		...
		this.tileStartX =
			tilemap.tileStartX ?? Math.round(this.x - this.width * this.anchorX)
		this.tileStartY =
			tilemap.tileStartY ??
			Math.round(this.y - this.height * this.anchorY)
		this.tileEndX = tilemap.tileEndX ?? this.tileStartX + this.width
		this.tileEndY = tilemap.tileEndY ?? this.tileStartY + this.height
		this.tiles = tilemap.code
			? Codec.decodeTiles(tilemap.code, this.width, this.height)
			...
			switch (tileData.type) {
				case 'normal': {
					...
					if (guid && texture === undefined) {
						texture = GL.createImageTexture(guid, {
				...
				switch (tileset.type) {
					case 'normal': {
						...
						this.tileData[dataId] = {
							...
							tag: tileset.tags[id],
							priority:
								tileset.priorities[id] + tileset.globalPriority
						}
						return
					}
					case 'auto': {
		...
		for (let y = top; y < bottom; y++) {
			for (let x = left; x < right; x++) {
				if (x >= 0 && x < width && y >= 0 && y < height) {
					...
					const neighbor =
						(((x > 0 &&
							key !== tiles[ti - 1] >> 8) as unknown as number) +
							1) |
						((((x > 0 &&
							y > 0 &&
							key !==
						...
						((((y > 0 &&
							key !==
								tiles[ti - width] >> 8) as unknown as number) +
							1) <<
							4) |
						((((x < r &&
							y > 0 &&
	...
	public draw(): void {
		...
		if (endIndex !== 0) {
			...
			const matrix = Matrix.instance
				.project(gl.flip, Camera.width, Camera.height)
	...
	public static lightSamplingModes: TilemapLightSamplingMap = {
		raw: 0,
		global: 1,
		ambient: 2
	}
...
class ScenePartitionManager<T> {
	...
	public shrink(scene: SceneContext, colliderSize: number): void {
		if (
			this.size / 2 >= colliderSize &&
			this.shiftBits > this.minShiftBits
		) {
			while (
				this.size / 2 >= colliderSize &&
				this.shiftBits > this.minShiftBits
	...
	public update(object: ObjectInCell): void {
		...
		if (object.cellId !== cellId) {
			if (object.cellId !== -1) {
				...
				const idx = cell.indexOf(object as T)
				if (idx !== -1) {
...
class SceneActorManager {
	...
	public update(deltaTime: number): void {
		...
		for (let i = 0, length = list.length; i < length; i++) {
			...
			if (collider.immovable) hasImmovable = true
			const size = collider.size
			if (maxGridCellSize < size) {
				if (maxColliderSize < size) {
					maxColliderSize = size
				}
				if (collider.weight !== 0) {
					maxGridCellSize = size
					hasAnyWeight = true
				}
			}
		}
		ActorCollider.hasImmovableActors = hasImmovable
		ActorCollider.hasCollisionActors = hasAnyWeight
		this.setMaxColliderSize(maxColliderSize)
		this.setMaxGridCellSize(maxGridCellSize)
		if (hasAnyWeight) ActorCollider.handleActorCollisions()
...
class SceneRegion {
	...
	public emit(type: string, triggerActor?: Actor): void {
		...
		if (triggerActor instanceof Actor) {
			this.script
				.getEvents(type)
				?.call(new ScriptRegionEvent(triggerActor!, this))
		} else {
			this.script.emit(type, this)
		}
	}
...
class SceneLight {
	...
	private drawAreaLight(projMatrix: Matrix, opacity: number): void {
		...
		const green = (this.green * opacity) / 255
		const blue = (this.blue * opacity) / 255
		const matrix = gl.matrix.set(projMatrix).rotateAt(ox, oy, this._angle)
		gl.bindVertexArray(program.vao.a110)
	...
	public move(
		...
		// 转换属性词条的数据结构
		const propEntries = Object.entries(properties) as Array<
			[string, number]
		>
		// 允许多个过渡同时存在且不冲突
		const { updaters } = this
			...
			if (a === 0) {
				...
				for (let i = 0; i < length; i++) {
					...
					if (
						...
						const key = max(
							0,
							min(0x3ffff, round((py + p) * 0x20000 + 0x10000))
						)
						datau[di] = 0x10000 | a
						datau[di + 1] = count
						if (starts[key] === 0) {
			...
			for (let i = 0; i < li; i++) {
				...
				do {
					...
					if (code1 < 0x10000) {
						...
						const light =
							modeMap[
								tilemap.light as keyof AnimationLightSamplingModes
							] << 16
...
let PathFinder = new (class ScenePathFinder {
	...
	/** 相邻图块的八个方向坐标偏移值列表 */
	private offsets: Int32Array = new Int32Array([
	...
	public initialize(): void {
		...
		this.vertices = new Float64Array(
			GL.arrays[0].float64.buffer,
			0,
			512 * 512 * 6
		)
		this.openset = new Uint32Array(
			GL.zeros.buffer,
			512 * 512,
			(512 * 512 * 2) / 4
		)
	...
	public createPath(
		...
		// 设置寻路阈值，期望值达到阈值后放弃计算
		PathFinder.threshold =
			Math.dist(sx, sy, dx, dy) + PathFinder.maxExtraCost
		// 打开起点
			...
			for (let i = 0; i < PathFinder.queueCount; i++) {
				...
				for (let i = 0; i < 16; i += 2) {
					...
					if (tx >= 0 && tx < width && ty >= 0 && ty < height) {
						const cost = PathFinder.calculateExtraCostBetween(
							tx,
							ty,
							nx,
							ny,
						...
						if (
							cost === 0 &&
							PathFinder.isInLineOfSight(px, py, nx, ny, passage)
						) {
							// 如果相邻网格可通行，且与父节点可见
	...
	private buildPath(
		...
		if (radius !== 0.5) {
			...
			for (let i = ci + 2; i < end; i += 2) {
				...
				// 求中转点拐角平分线的弧度
				const radian =
					Math.abs(radian1 - radian2) < Math.PI
						? (radian1 + radian2) / 2
	...
	private setBypass(bypass: boolean): void {
		const useBypass = bypass && ActorCollider.hasCollisionActors
		if (useBypass) {
			this.calculateExtraCostBetween =
				PathFinder.bypassCalculateExtraCostBetween
			this.isInLineOfSight = PathFinder.bypassIsInLineOfSight
		} else {
			this.calculateExtraCostBetween =
				PathFinder.normalCalculateExtraCostBetween
	 ...
	 * @returns 额外的寻路成本
	 */
	private calculateExtraCostBetween!: (
		sx: number,
		sy: number,
		dx: number,
		dy: number,
	 ...
	 * @returns 额外的寻路成本
	 */
	private normalCalculateExtraCostBetween = (
		sx: number,
		sy: number,
		dx: number,
		dy: number,
		...
		if (
			PathFinder.terrains[dIndex] !== passage ||
	 ...
	 * @returns 额外的寻路成本
	 */
	private bypassCalculateExtraCostBetween = (
		sx: number,
		sy: number,
		dx: number,
		dy: number,
...
class EntityManager {
	...
	public get(key: string): EntityObject | undefined {
		return (
			this.presetIdMap[key] ?? this.nameMap[key] ?? this.entityIdMap[key]
		)
	}
