...
let Command = new (class CommandCompiler {
	...
	protected return({
		type,
		value
	}: GlobalEventResultGetter): CommandFunction {
	...
	private compileNumber(
		...
		switch (typeof number) {
			case 'number':
				return () => number
			case 'object': {
				const getNumber = Command.compileVariable(
					number,
					Attribute.NUMBER_GET
				)
	...
	private compileEnumValue(enumId: string | VariableGetter): () => string {
		switch (typeof enumId) {
			...
			case 'object': {
				const getString = Command.compileVariable(
					enumId,
	...
	public compileVariable = (function compileVariable(IIFE) {
		const compilers = {
			...
			equipment: (equipment: EquipmentGetter) =>
				Command.compileEquipment(equipment),
		...
		return function (
			...
			switch (type) {
				case 'local':
					return (value?: any) =>
						operation(CurrentEvent.attributes, key, value)
				case 'global':
					return (value?: any) => operation(Variable.map, key, value)
				case 'self':
					return (value?: any) =>
						CurrentEvent.selfVarId &&
						operation(
							SelfVariable.map,
							CurrentEvent.selfVarId,
							value
	...
	public compileActor(actor: ActorGetter): () => Actor | undefined {
		switch (actor.type) {
			case 'trigger':
				return () =>
					Command.filterValidObject(CurrentEvent.triggerActor)
			case 'caster':
			...
			case 'player':
				return () =>
					Command.filterValidObject(Party.player ?? undefined)
			case 'member': {
	...
	public compileEquipment(
		...
		switch (equipment.type) {
			...
			case 'by-id-equipped': {
				const getActor = Command.compileActor(equipment.actor)
				return () =>
					getActor()?.equipment.getById(equipment.equipmentId)
			}
			case 'by-id-inventory': {
				const getActor = Command.compileActor(equipment.actor)
				return () => {
					const asset = getActor()?.inventory.get(
	...
	private compileLight(light: LightGetter): () => SceneLight | undefined {
		switch (light.type) {
			case 'trigger':
				return () =>
					Command.filterValidObject(CurrentEvent.triggerLight)
			case 'latest':
	...
	private compileRegion(region: RegionGetter): () => SceneRegion | undefined {
		switch (region.type) {
			case 'trigger':
				return () =>
					Command.filterValidObject(CurrentEvent.triggerRegion)
			case 'by-id': {
	...
	private compileTilemap(
		...
		switch (tilemap.type) {
			case 'trigger':
				return () =>
					Command.filterValidObject(CurrentEvent.triggerTilemap)
			case 'by-id': {
				...
				case 'actor': {
					...
					switch (operation) {
						...
						case 'has-items': {
							...
							// @ts-ignore
							return () =>
								getActor()?.inventory.count(itemId)! >= quantity
						}
						case 'has-equipments': {
							const { equipmentId, quantity } = condition
							// @ts-ignore
							return () =>
								getActor()?.inventory.count(equipmentId)! >=
								quantity
						}
						case 'has-skill-shortcut': {
							const getKey = Command.compileEnumValue(
								condition.shortcutKey
							)
							return () =>
								getActor()?.shortcut.get(getKey())?.type ===
								'skill'
						}
						...
						case 'equipped': {
							...
							return () =>
								!!getActor()?.equipment.getById(equipmentId)
						}
						case 'is-teammate': {
							const getTarget = Command.compileActor(
								condition.target
							)
							return () => {
								const actor = getActor()
	...
	protected showChoices({
		...
		if (!parseVariable) {
			...
			const mapper = (content: string) =>
				content.replace(regexp, replacer)
			...
			switch (operand.type) {
				...
				case 'math':
					switch (operand.method) {
						case 'round': {
							...
							const getter = Command.compileVariable(
								operand.variable,
								NUMBER_GET
							)
						...
						case 'sqrt':
						case 'abs': {
							const mathMethod = Math[
								operand.method as keyof Math
							] as any
							const getter = Command.compileVariable(
								operand.variable,
								NUMBER_GET
						...
						case 'tan': {
							...
							const getAngle = Command.compileVariable(
								operand.variable,
								NUMBER_GET
							)
						...
						case 'distance': {
							const { dist } = Math
							const getStart = Command.compilePosition(
								operand.start
							)
				...
				case 'string': {
					...
					switch (operand.method) {
						...
						case 'search': {
							const getSearch = Command.compileString(
								operand.search
							)
							return () =>
								getter()?.indexOf(getSearch() || undefined)
						}
					}
				}
				case 'object':
					switch (operand.property) {
						...
						case 'actor-inventory-equipment-quantity': {
							const getActor = Command.compileActor(operand.actor)
							const getId = Command.compileString(
								operand.equipmentId
							)
							return () => getActor()?.inventory.count(getId())
						}
						...
						case 'equipment-order': {
							const getEquipment = Command.compileEquipment(
								operand.equipment
							)
						...
						case 'trigger-speed': {
							const getTrigger = Command.compileTrigger(
								operand.trigger
							)
						...
						case 'tilemap-width': {
							const getTilemap = Command.compileTilemap(
								operand.tilemap
							)
						...
						case 'element':
							switch (key) {
								...
								case 'index-of-selected-button':
									return () =>
										UI.getIndexOfSelectedButton(
											getElement()
										)
							}
						case 'transform':
							return () =>
								getElement()?.transform[key as TransformKey]
						case 'window':
							switch (key) {
				...
				case 'other':
					switch (operand.data) {
						...
						case 'trigger-wheel-y':
							return () =>
								Input.event instanceof ScriptWheelEvent
									? Input.event.deltaY
									: 0
						...
						case 'mouse-scene-y':
							return () => Mouse.sceneY
						case 'touch-screen-x': {
							const getTouchId = Command.compileNumber(
								operand.touchId
							)
						...
						case 'actor-count':
							return () =>
								Scene.binding?.actor.count(operand.teamId) ?? 0
		...
		const compileOperands = (
			...
			for (let i = 0; i < length; i++) {
				...
				const operation = operand.operation.replace(
					'()',
					''
				) as keyof typeof operationPriorityMap
	...
	protected setString = (function (IIFE) {
		...
		const compileOperand = (operand: any): (() => string | undefined) => {
			switch (operand.type) {
				...
				case 'string':
					switch (operand.method) {
						case 'char': {
							...
							const getIndex = Command.compileNumber(
								operand.index,
								-1
							)
						...
						case 'slice': {
							...
							const getBegin = Command.compileNumber(
								operand.begin
							)
						...
						case 'replace-all': {
							...
							const getPattern = Command.compileString(
								operand.pattern,
								defaultString
							)
							const getReplacement = Command.compileString(
								operand.replacement,
								defaultString
							)
							return () => {
				...
				case 'attribute': {
					const string =
						Attribute.getKey(operand.attributeId) || undefined
					return () => string
				}
				case 'enum': {
				...
				case 'other':
					switch (operand.data) {
						case 'trigger-key':
							return () =>
								Input.event instanceof ScriptKeyboardEvent
									? Input.event.keyName
									: undefined
						...
						case 'showChoices-content': {
							...
							return () =>
								Command.choiceContents[getIndex()] ?? ''
						}
						case 'parse-timestamp': {
							const getTimestamp = Command.compileNumber(
								operand.variable,
								-1
							)
							...
							return () => {
								...
								if (timestamp !== -1) {
									return Time.parseDateTimestamp(
										timestamp,
										format
									)
								}
						...
						case 'screenshot': {
							...
							return () =>
								GL.offscreen.current.toBase64(width, height)
						}
						case 'game-language':
	...
	protected setList = (function (IIFE) {
		...
		return function ({
			...
			switch (operation) {
				case 'set-empty': {
					const setList = Command.compileVariable(
						variable,
						OBJECT_SET
					)
					return () => {
				...
				case 'set-variable': {
					...
					return () => {
						...
						if (
							...
							list[index!] = value
						} else if (!skipCheck && value !== undefined) {
							list[index!] = value
						}
						return true
					}
				...
				case 'split-string': {
					...
					const getSeparator = Command.compileString(
						separator!,
						'$INVALID_VARIABLE$'
					)
					return () => {
	...
	protected switch = (function (IIFE) {
		...
		const compileCondition = (condition: any): (() => boolean) => {
			switch (condition.type) {
				...
				case 'attribute': {
					const string =
						Attribute.getKey(condition.attributeId) || null
					return () => Value === string
				}
				case 'enum': {
		...
		return function ({
			...
			if (length < 6) {
				...
				switch (length) {
					case 1:
						return () => ((Value = getter()), a() ? f : defFn)()
					case 2:
						return () =>
							((Value = getter()), a() ? f : b() ? g : defFn)()
					case 3:
						return () =>
							((Value = getter()),
							a() ? f : b() ? g : c() ? h : defFn)()
	...
	protected forEach = (function forEach(IIFE) {
		...
		// 编译角色组件列表
		const compileActorComponentList = (
			actor: ActorGetter,
			data: string
		) => {
			const getActor = Command.compileActor(actor)
		...
		return function ({
			...
			switch (data) {
				...
				case 'changed-touch':
					getList = () => {
						return Input.event instanceof ScriptTouchEvent
							? Input.event.changedTouches.map(
									(touch) => touch.id
								)
							: []
					}
			...
			switch (data) {
				default: {
					const iterator = compileCommonIterator(
						variable ?? touchId!,
						foreach
					)
					const loopCommands = Command.compile(
			...
			for (const arg of eventArgs!) {
				if (arg.key === key && arg.type === type) {
					...
					switch (type) {
						...
						case 'number':
							getters.push(
								Command.compileNumber(
									value as number | VariableGetter
								)
							)
							setters.push(
								Command.compileVariable(
						...
						case 'string':
							getters.push(
								Command.compileString(
									value as string | VariableGetter
								)
							)
							setters.push(
								Command.compileVariable(
						...
						case 'actor':
							getters.push(
								Command.compileActor(value as ActorGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'skill':
							getters.push(
								Command.compileSkill(value as SkillGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'state':
							getters.push(
								Command.compileState(value as StateGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'equipment':
							getters.push(
								Command.compileEquipment(
									value as EquipmentGetter
								)
							)
							setters.push(
								Command.compileVariable(
						...
						case 'item':
							getters.push(
								Command.compileItem(value as ItemGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'trigger':
							getters.push(
								Command.compileTrigger(value as TriggerGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'light':
							getters.push(
								Command.compileLight(value as LightGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
						...
						case 'element':
							getters.push(
								Command.compileElement(value as ElementGetter)
							)
							setters.push(
								Command.compileVariable(
									variable,
	...
	private compileReturnValueAccessors(
		...
		switch (eventResult.type) {
			...
			case 'actor': {
				...
				setReturnValue = () => {
					if (result instanceof Actor || result === undefined)
						setter(result)
				}
				break
			}
			case 'skill': {
				...
				setReturnValue = () => {
					if (result instanceof Skill || result === undefined)
						setter(result)
				}
				break
			}
			case 'state': {
				...
				setReturnValue = () => {
					if (result instanceof State || result === undefined)
						setter(result)
				}
				break
			}
			...
			case 'item': {
				...
				setReturnValue = () => {
					if (result instanceof Item || result === undefined)
						setter(result)
				}
				break
			}
			case 'trigger': {
				...
				setReturnValue = () => {
					if (result instanceof Trigger || result === undefined)
						setter(result)
				}
				break
			}
	 ...
	 * @returns 两个指令函数
	 */
	private compileCommandTupleInNamespace(
		commands: CommandFunctionList
	): [CommandFunction, CommandFunction] {
			...
			case 'global':
				switch (operation) {
					case 'register': {
						...
						const commandList = Command.compileIndependent(
							commands!
						)
	...
	protected transition({
		...
		const setNumber = Command.compileVariable(
			variable,
			Attribute.NUMBER_SET
		)
		...
		if (!setImageClip) {
			setImageClip = method.setImageClip = (
				...
				if (element instanceof ImageElement && object) {
					element.setImageClip(
						object.icon ?? object.portrait,
						object.clip
					)
					element.resize()
				}
	...
	protected setAnimation({
		...
		for (const property of properties) {
			switch (property.key) {
				...
				case 'frame':
					if (typeof property.value === 'object') {
						property.value = Command.compileNumber(
							property.value,
							0,
							0,
							10000
	...
	protected setObjectAnimation({
		...
		switch (operation) {
			...
			case 'set-offsetY':
				...
				break
			case 'set-rotation': {
				const getDegrees = Command.compileNumber(
					rotation!,
					0,
					-10000,
					10000
				...
				return () => {
					...
					if (actor) {
						if (tintProps) {
							...
							actor.setOpacityForAll(
								getOpacity(),
								easingId,
								duration
							)
						} else if (getOffsetY) {
							actor.setOffsetYForAll(
								getOffsetY(),
								easingId,
								duration
							)
						} else if (getRotation) {
							actor.setRotationForAll(
								getRotation(),
								easingId,
								duration
							)
	...
	protected changeActorMotion({
		...
		return !motionName
			? null
			: () => {
					getActor()?.animationController.changeMotion(
						type,
						motionName
					)
					return true
		...
		switch (operation) {
			...
			case 'set-speed': {
				...
				return () => {
					...
					if (animation)
						animation.speed = Math.clamp(getSpeed(), 0, 10)
					return true
				}
			}
			case 'set-opacity': {
				...
				return () => {
					const animation = getActor()?.animationManager.get(key)
					if (animation)
						animation.opacity = Math.clamp(getOpacity(), 0, 1)
					return true
				}
			}
			case 'set-priority':
			...
			case 'set-sprite':
				return () => {
					getActor()?.animationManager.setSprite(
						key,
						spriteId!,
						image!
					)
	...
	protected useItem({
		...
		switch (mode) {
			...
			case 'by-id':
				getItem = () =>
					getActor()?.inventory.get(itemId!) as Item | undefined
				break
			case 'by-item': {
	...
	protected commandLine({
		...
		variable: VariableGetter
		key: string | VariableGetter
		value: string | VariableGetter
	}): CommandFunction {
		const getKey = Command.compileString(key)
		const getValue = Command.compileString(value)
		const setVariable = Command.compileVariable(
			variable,
			Attribute.OBJECT_SET
		)
		return () => {
			if (Stats.shell !== 'electron') {
				console.warn('Command Line only works in electron')
				return false
			}
			...
			switch (type) {
				...
				case 'set':
					...
					return true
				case 'rm':
					ipcRenderer.send('remove-command-line-switch', getKey())
					return true
				case 'has':
					setVariable(
						ipcRenderer.sendSync(
	...
	/** 重启应用 */
	protected relaunchApp(): CommandFunction {
		return () => {
			if (Stats.shell !== 'electron') {
				console.warn('Command Line only works in electron')
			} else {
