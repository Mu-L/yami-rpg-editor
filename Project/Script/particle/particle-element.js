...
Particle.Element = class ParticleElement {
	...
	initialize() {
		...
		this.hRotationRadius =
			ParticleElement.getRandomParameter(hRotation.radius) * emitter.scale
	...
	update(deltaTime) {
		...
		switch (this.spriteMode) {
			...
			case 'animation':
				if (
					this.spriteFrame < this.spriteCount - 1 &&
	...
	setStartPositionEdge(movementAngle) {
		...
		if (random < threshold) {
			...
			this.x -= forward ? vertices[2] - this.x : vertices[0] - this.x
		}
	}
