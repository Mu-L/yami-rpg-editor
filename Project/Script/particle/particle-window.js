...
Particle.drawCoordinateAxes = function () {
	if (this.showAxes) {
		...
		const matrix = gl.matrix.set(Particle.matrix)
		// 避免缩放时虚线抖动
...
Particle.drawEmitterAnchor = function () {
	...
	vertices[6] = x + 0.5
	vertices[7] = y + 0.5 + 9
	gl.matrix.project(gl.flip, gl.width, gl.height)
	gl.alpha = 1
	gl.blend = 'normal'
	const program = gl.graphicProgram.use()
...
Particle.drawElementAnchors = function () {
	...
	for (const layer of this.emitter.layers) {
		...
		for (let i = 0; i < count; i++) {
			const element = elements[i]
			matrix.set(Particle.matrix).translate(element.x, element.y)
			const x = matrix[6]
			const y = matrix[7]
			vertices[vi] = x + 0.5 - 8
			vertices[vi + 1] = y + 0.5
	...
	if (dragging.relate(event)) {
		switch (dragging.mode) {
			case 'object-move': {
				...
				const dpr = window.devicePixelRatio
				const distX =
					((event.clientX - dragging.clientX) * dpr) / Particle.scaleX
				const distY =
					((event.clientY - dragging.clientY) * dpr) / Particle.scaleY
...
Particle.list.createIcon = function (item) {
	...
	if (path) {
		...
		FileBodyPane.prototype.setIconClip(
			icon,
			path,
			0,
			0,
