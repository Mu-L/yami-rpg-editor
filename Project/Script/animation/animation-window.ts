...
import { Curve } from './curve-window.ts';
...
Animation.initialize = function () {
	...
	History.processors['animation-layer-create'] =
		...
		History.processors['animation-layer-remove'] =
			(operation, data) => {
				const { motion, direction, response } = data;
...
Animation.updateMarqueeShift = function (x, y, length) {
	...
	if (length !== undefined && marquee.length !== length) {
		marquee.length = length;
		marquee.style.width = `${length * 16}px`;
	}
};
...
Animation.loadTextures = function () {
	...
	for (const sprite of this.sprites) {
		...
		if (texture instanceof ImageTexture && texture.base.guid === imageId) {
			const { base } = texture;
...
Animation.drawOnionskins = function () {
	if (this.showOnionskin) {
		GL.alpha = 0.25;
		const { prev, next } = this.player.onionskin;
...
Animation.drawSpriteLayers = function (contexts = this.player.contexts) {
	...
	for (let i = 0; i < count; i++) {
		const context = contexts[i];
		const { layer, frame } = context;
		if (layer.class === 'sprite' && !layer.hidden && frame !== null) {
			const key = layer.sprite;
...
Animation.drawEmitters = function () {
	...
	for (let i = 0; i < count; i++) {
		const context = contexts[i];
		const { layer, frame } = context;
		if (layer.class === 'particle' && !layer.hidden && frame !== null) {
			matrix.set(Animation.matrix).multiply(context.matrix);
			let vi = 0;
			const x = matrix[6];
			const y = matrix[7];
...
Animation.drawJointNodes = function () {
	...
	for (let i = 0; i < count; i++) {
		const context = contexts[i];
		const { layer, frame } = context;
		if (layer.class === 'joint' && !layer.hidden && frame !== null) {
			matrix.set(Animation.matrix).multiply(context.matrix);
			let vi = 0;
			const x = matrix[6];
			const y = matrix[7];
...
Animation.drawJointArrows = function () {
	...
	for (let i = 0; i < count; i++) {
		...
		if (
			...
			matrix.set(Animation.matrix).multiply(parent.matrix);
			const sx = matrix[6];
			const sy = matrix[7];
			matrix.set(Animation.matrix).multiply(context.matrix);
			const dx = matrix[6];
...
Animation.drawHoverWireframe = function () {
	...
	if (hover !== null && hover !== this.target) {
		...
		for (let i = 0; i < count; i++) {
			const context = contexts[i];
			if (context.frame === hover && !context.layer.hidden) {
			...
			if (frame !== null) {
				...
				while (parent !== null) {
					if (parent.frame !== null) {
						if (parent.layer.hidden || parent.layer.locked) {
							parent = null;
						}
						break;
					}
...
Animation.isPointInFrame = function (context, x, y) {
	...
	const matrix = GL.matrix.set(context.matrix);
	const L = -(texture.width * context.anchorX + context.pivotX);
	const T = -(texture.height * context.anchorY + context.pivotY);
	const R = L + texture.width;
	const B = T + texture.height;
...
Animation.getAnimationRange = (function IIFE() {
	...
	return function () {
		...
		if (marquee.y !== -1) {
			...
			range.end = length === 1 ? end : Math.min(x + length, end);
		} else {
			range.start = 0;
			range.end = end;
		}
...
Animation.switchLoop = (function IIFE() {
	...
	return function (enabled = !this.loop) {
		...
		this.loop = enabled;
		// 在开启残影的情况下应该刷新
		if (this.showOnionskin && this.state === 'open') {
			this.updateFrameContexts();
			this.requestRendering();
		}
	};
...
Animation.outerTimelineListKeydown = function (event) {
	...
	if (event.cmdOrCtrlKey) {
		...
		switch (event.code) {
			...
			case 'Home':
			case 'End':
				Animation.multiSelectFramesToHomeEnd(event.code.toLowerCase());
				break;
		}
	} else {
		switch (event.code) {
			...
			case 'End':
				event.preventDefault();
				Animation.selectFrameAtHomeEnd(event.code.toLowerCase());
				break;
			case 'PageUp':
				event.preventDefault();
				break;
